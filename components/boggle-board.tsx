...
export default function BoggleBoard({
  ...
  const handleLetterChange = (
    ...
    const newBoard = editBoard.map((row, r) =>
      row.map((cell, c) => (r === rowIndex && c === colIndex ? letter : cell))
    );
  ...
  const handleSaveChanges = () => {
    ...
    const validBoard = editBoard.map((row) =>
      row.map((cell) => (cell.trim() === '' ? 'e' : cell))
    );
      ...
      <div className="boggle-board relative grid grid-cols-4 gap-2 aspect-square w-full max-w-md mx-auto">
          ...
          row.map((letter, colIndex) => (
            <div
              ...
              className={cn(
                ...
                !editMode &&
                  isFirstInPath(rowIndex, colIndex) &&
                  'first-in-path'
              )}>
              {editMode ? (
                <input
        ...
        {loading && (
          <div className="absolute inset-0 bg-background/80 backdrop-blur-sm flex flex-col items-center justify-center z-30 rounded-lg">
            ...
            <p className="mt-4 text-boggle-accent font-medium">
              {loadingMessage}
            </p>
          </div>
        )}
